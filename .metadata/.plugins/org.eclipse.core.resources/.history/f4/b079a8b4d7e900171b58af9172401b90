...
public class DriveF extends Command {
    ...
    public DriveF(double sec, double power) {
    	...
    	this.sec = sec;//Sets the class level variable 'sec' to what the command is given to initialize
    	this.power=power;//Sets the class level variable 'power' to what the command is given to initialize
    	timer=new Timer();//Sets the class level variable 'timer' to a new instance of a Timer
    	done=false;//Sets the class level boolean 'done' to false, which says that the command isn't done running yet (changed in end() later on)
    }
    ...
    protected void end() {
		...
		done = true;//Sets done to 'true', which allows isFinished() to return true, meaning 'The command is done'
    }
